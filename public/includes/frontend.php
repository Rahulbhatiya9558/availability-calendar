    ...
    {	
		...
		*/
			for($m=$month_cur;$m<=$endmonth_cur;$m++){
				...
				if (mb_check_encoding($mName, 'UTF-8')) {
					$encoding = mb_detect_encoding($mName,'auto' );
					$monthName =mb_convert_encoding($mName, $encoding,'auto');
				}else{
					$monthName =utf8_encode($mName);
				}
		...
		*/
				...
				for( $i=1; $i<= $num_of_days; $i++){
					...
					if (mb_check_encoding($dName, 'UTF-8')) {
						$encoding = mb_detect_encoding($dName,'auto' );
						$dayName =mb_convert_encoding($dName, $encoding,'auto');
					}else{
						$dayName =utf8_encode($dName);
					}
					...
					if(!empty($day)){
						...
						if($settings_options['old_date_fade_out'] == 'yes'){
							...
							if(!empty($settings_options['old_date_fade_out_color'])){
								$old_date_fade_out_color = sanitize_hex_color_no_hash($settings_options['old_date_fade_out_color']);
							}
